"""
안성시도서관 searchView.do?mId=... 파라미터 테스트
"""
import requests
...
mIds = ["0101030000", "0101010000", "0101000000"]
...
for mId in mIds:
    ...
    try:
        ...
        print(f"  GET Status: {r.status_code}, Length: {len(r.text)}, '파이썬' count: {r.text.count('파이썬')}")
        if r.status_code == 200:
            with open(f"anseong_mId_{mId}.html", "w", encoding="utf-8") as f:
                f.write(r.text)
    except Exception as e:
        print(f"  Error: {e}")

    print(f"=== POST {url} mId={mId} ===")
    try:
        r = session.post(url, data=params, headers=HEADERS, timeout=8, verify=False)
        print(f"  POST Status: {r.status_code}, Length: {len(r.text)}, '파이썬' count: {r.text.count('파이썬')}")
    except Exception as e:
        print(f"  Error: {e}")
