"""
파주시도서관(lib.paju.go.kr/intro/index.do) GNB 및 검색 폼 분석
"""
import requests
...
url = "https://lib.paju.go.kr/intro/index.do"

try:
    r = session.get(url, headers=HEADERS, timeout=8, verify=False)
    print("Status:", r.status_code)
    
    with open("paju_main.html", "w", encoding="utf-8") as f:
        f.write(r.text)
    print("Saved paju_main.html")
    ...
    print(f"\n=== 파주시 도서검색 관련 링크 ({len(a_search)}개) ===")
    for j, a in enumerate(a_search[:15]):
